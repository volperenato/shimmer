...
public:
	...
	void setDecayInSeconds(float decay) {
		...
		// Compute mean delay value
		for (int i = 0; i < mcf_numberOfChannels; i++)
			del += mcf_DelayLines[i]->getDelayLength();
		del /= mcf_numberOfChannels;

		// Compute feedback based on the mean delay value and desired decay in seconds
		float feedbackModule = (decay > 0.0) ? pow(10, -3.0 * del / (decay * 1000.0)) : 0.0;

		// Set computed feedback value
		for (int i = 0; i < mcf_numberOfChannels; i++)
			mcf_DelayLines[i]->setFeedback(feedbackModule);
	}	
